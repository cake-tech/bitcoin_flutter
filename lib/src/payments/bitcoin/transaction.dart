...
class BtcTransaction {
  BtcTransaction({
    ...
    bool? hasTaproot,
        ...
        version = hasTaproot == true
            ? Uint8List.fromList(TX_VERSION_2)
            : (v ?? Uint8List.fromList(DEFAULT_TX_VERSION)) {
    witnesses.addAll(w);
  }
  ...
  static BtcTransaction fromRaw(String raw) {
    ...
    if (hasSegwit) {
      for (int n = 0; n < inputs.length; n++) {
        ...
        for (int n = 0; n < wVi.$1; n++) {
          ...
          if (wtVi.$1 != 0) {
            witness = rawtx.sublist(cursor + wtVi.$2, cursor + wtVi.$1 + wtVi.$2);
          }
    ...
    return BtcTransaction(inputs: inputs, outputs: outputs, w: witnesses, hasSegwit: hasSegwit);
  }
  ...
  Uint8List getTransactionDigest(
      {required int txInIndex, required Script script, int sighash = SIGHASH_ALL}) {
    final tx = copy(this);
    ...
    if ((sighash & 0x1f) == SIGHASH_NONE) {
      ...
      if (txInIndex >= tx.outputs.length) {
        throw ArgumentError("Transaction index is greater than theavailable outputs");
      }
      final txout = tx.outputs[txInIndex];
      tx.outputs.clear();
      for (int i = 0; i < txInIndex; i++) {
        tx.outputs
            .add(TxOutput(amount: BigInt.from(NEGATIVE_SATOSHI), scriptPubKey: Script(script: [])));
      }
  ...
  Uint8List getTransactionSegwitDigit(
    ...
    bool signAll = (basicSigHashType != SIGHASH_SINGLE) && (basicSigHashType != SIGHASH_NONE);
    if (!anyoneCanPay) {
      hashPrevouts = Uint8List(0);
      for (final txin in tx.inputs) {
        Uint8List txidBytes = Uint8List.fromList(hexToBytes(txin.txId).reversed.toList());
    ...
    if (signAll) {
      ...
      for (final i in tx.outputs) {
        ...
        hashOutputs = Uint8List.fromList(
            [...hashOutputs, ...amountBytes, scriptBytes.length, ...scriptBytes]);
      }
      hashOutputs = doubleHash(hashOutputs);
    } else if (basicSigHashType == SIGHASH_SINGLE && txInIndex < tx.outputs.length) {
      ...
      hashOutputs = Uint8List.fromList([...packedAmount, ...lenScriptBytes, ...scriptBytes]);
      hashOutputs = doubleHash(hashOutputs);
    }
  ...
  Uint8List getTransactionTaprootDigset(
    ...
    if (!anyoneCanPay) {
      ...
      for (final txIn in newTx.inputs) {
        hashSequences = Uint8List.fromList([...hashSequences, ...txIn.sequence]);
      }
